...
namespace HlidacStatu.Lib.Data.External
{
    public class Merk
    {
        ...
        private static CoreCompanyStructure[] Search(string url, string query)
        {
            try
            {
                CoreCompanyStructure[] res = Newtonsoft.Json.JsonConvert.DeserializeObject<CoreCompanyStructure[]>(
                    Request(string.Format(url, System.Net.WebUtility.UrlDecode(query)))
                    );
                if (res == null)
