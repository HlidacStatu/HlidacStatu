    ...
    selection.each(function drawGraph(dataset) {
      ...
      g.selectAll('rect')
          ...
          .on('mouseover', function (d, i) {
            ...
            divTooltip.transition()
                .duration(200)
                .style('opacity', 0.9);
            divTooltip.html(function () {
              var output = '';
              if (customCategories) {
                // custom categories: display category name
                output = '<div class="tooltip inner">' + d[1] + '&nbsp;';
              } else {
                if (d[1] === 1) {
                  // checkmark icon
              ...
              if (isDateOnlyFormat) {
                if (d[2] > d3.time.second.offset(d[0], 86400)) {
                  return output + moment(parseDate(d[0])).format('dd l')
                      + '&nbsp;-&nbsp;' + moment(parseDate(d[2])).format('l') + '</div>';
                }
                return output + moment(parseDate(d[0])).format('l') + '</div>';
              } else {
                if (d[2] > d3.time.second.offset(d[0], 86400)) {
                  return output + moment(parseDateTime(d[0])).format('l') + ' '
                      + moment(parseDateTime(d[0])).format('LT') + '&nbsp;-&nbsp;'
                      + moment(parseDateTime(d[2])).format('l') + '&nbsp;'
                      + moment(parseDateTime(d[2])).format('LT') + '</div>';
                }
                return output + moment(parseDateTime(d[0])).format('dd LT') + '&nbsp;-&nbsp;'
                    + moment(parseDateTime(d[2])).format('dd LT')+'</div>';
              }
            })
            .style('left', function () {
            ...
            .style('top', function () {
              return window.pageYOffset + matrix.f - 19 + 'px';
            })
      ...
      if (isDateOnlyFormat) {
          ...
          subtitleText = 'od ' + moment(parseDateTime(startDate)).format('l') + ' '
              ...
              + moment(parseDateTime(endDate)).format('LT');
      }
