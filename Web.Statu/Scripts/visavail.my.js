function visavailChart() {
    ...
    // title of chart
    var title = 'Data Availability Plot';
    ...
    function chart(selection) {
        selection.each(function drawGraph(dataset) {
            ...
            var xAxis = d3.svg.axis()
                .scale(xScale)
                .tickFormat(CZ.timeFormat("%a %H:%M"))
                .orient('top');
            ...
            function zoomed() {
                ...
                tx = Math.max(Math.min(0, tx), width * (1 - d3.event.scale));
                zoom.translate([tx, 0]);
                gData.attr("transform", "translate(" + tx + ",0) scale(" + d3.event.scale + ", 1)");
                gXAxis.call(xAxis);
            }
            ...
            var gClip = svg.append('g')
                .attr('id', 'g_clipped')
                .attr('clip-path', 'url(#clip)')

            gClip.append('rect').attr('width', width).attr('height', height).attr('display', 'none');
            gClip.call(zoom);

            var gData = gClip.append('g').append('g').attr('id', 'g_data');
            ...
            // create x axis
            var gXAxis = svg.select('#g_axis').append('g')
                .attr('class', 'axis')
                .call(xAxis);
            ...
            g.selectAll('rect')
                ...
                .on('mouseover', function (d, i) {
                    ...
                    div.transition()
                        ...
                        .style('opacity', 0.9);
                    div.html(function () {
                        var output = '<span id="currenTime"></span>';
                        if (customCategories) {
                ...
                .on("mousemove", function () {
                    d3.select('#currenTime').html(moment(xScale.invert(d3.event.pageX - margin.left - 8)).format('LTS') + ' - ');
            ...
            var subtitleText = '';
            if (isDateOnlyFormat) {
                subtitleText = 'od ' + moment(parseDate(startDate)).format('MMMM Y') + ' do '
                    + moment(parseDate(endDate)).format('MMMM Y');
            } else {
                subtitleText = 'od ' + moment(parseDateTime(startDate)).format('l') + ' '
                    + moment(parseDateTime(startDate)).format('LT') + ' do '
