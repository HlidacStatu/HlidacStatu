...
{
    public class Worker_Puppeteer : IDisposable
    {

        HlidacStatu.Util.WebShot.PuppeteerBrowser chrome = null;
        Thread t = null;
        public string WorkerName = "";
        public Worker_Puppeteer(string workerName, CancellationToken cancelToken)
        {
            this.WorkerName = workerName;
            chrome = new PuppeteerBrowser(workerName);
            t = new Thread(() =>
            {
                chrome.WarmUp();
                HlidacStatu.Util.InfinitiveTask.Run((tag) =>
                {
                    ...
                    if (Workers.UrlToProcess.TryDequeue(out swr))
                    {
                        ...
                        if (wr != null)
                        {
                            ...
                            byte[] result = Url2PngSingle_Puppeteer.Screenshot(chrome, wr.Url, wr.cropWidth, wr.cropHeight);
